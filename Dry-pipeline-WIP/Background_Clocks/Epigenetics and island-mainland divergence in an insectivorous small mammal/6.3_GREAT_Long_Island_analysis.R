## R Script for GREAT analysis (to find gene pathways) for Long Island vs other populations
...
tab <- read.csv('EWAS/EbFit_LongIsland_effect.csv')
...
write.csv(output.all,"output.pos_LongIsland.csv")
...
saveRDS(output.all,file=output.rds)
#save as csv on computer
write.csv(output.all,"output.neg_LongIsland.csv")
...
mouse_pos_top <- top_n(mouse_pos_df, -1, HyperBonfP)
mouse_neg_top <- top_n(mouse_neg_df, -4, HyperBonfP)
...
whole_df <- rbind(GO_bio_df, Go_mol_df, InterPro_df, mouse_df)
...
write.csv(whole_df,file="GREAT_df_LongIsland.csv")
...
####FINAL PLOT (FIGURE S3 IN SUPPLEMENTAL)
#load cowplot to be able to merge all 4 plots in one figure
library(cowplot)

#to create this final figure you will need to have run the 4.3_EWAS_Long_Island_effect script and have the "DNAm_Location_plot" in your environment
#you will also need to have run the 5.3_Manhattan_plot_Long_Island_effect script and have the "manhplot" plot in your environment

#make the left part of Figure S3 with the manhattan plot and boxplots
...
Figure_S3 <- plot_grid(left, whole_df_plot , labels = c('',  'C'), cols = 2, rel_widths = c(2, 1.25))
#view final Figure S3 and manually export as pdf
Figure_S3
