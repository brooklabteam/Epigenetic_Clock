##R Script for EWAS Manhattan plot (Bon Portage Island vs other populations)
...
tab <- read_csv("EWAS/EbFit_BPI_effect.csv")
...
#save to computer
write.csv(allData_manhattan,file="allData_manhattan_sorexcinereus_BPI.csv")
...
manhplot <- ggplot(allData_manhattan, aes(x = bp_cum, y = -log10(p))) +
  ...
  #annotation info for top n (want 15 but many don't have gene annotations so select 22 to get 15)
  #add label for gene name, make font size 5 and black
  geom_text_repel(data=allData_manhattan %>% top_n(-22, p), aes(label=gene), size=5, color ="black") +
  #place ticks in middle of each chromosome/scaffold on x axis
  scale_x_continuous(label = axis_set$chr, breaks = axis_set$center) +
  #y axis limits set and breaks based on p values
  #start at 0 till 24 but labels start at 4 till 20 with jumps of 5
  scale_y_continuous(expand = c(0,0), limits = c(0, ylim), breaks = seq(5, 45, by = 10)) +
  ...
  #title 
  ggtitle("EWAS of Bon Portage Island Effect") +
