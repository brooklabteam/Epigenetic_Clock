## R Script for GREAT analysis (to find gene pathways) for island vs mainland
...
tab <- read.csv('EWAS/EbFit_Island_effect.csv')
...
write.csv(output.all,"output.pos_location.csv")
...
saveRDS(output.all,file=output.rds)
#save as csv on computer
write.csv(output.all,"output.neg_location.csv")
...
mouse_pos_top <- top_n(mouse_pos_df, -6, HyperBonfP)
mouse_neg_top <- top_n(mouse_neg_df, -1, HyperBonfP)
...
Go_mol_pos_top <- top_n(Go_mol_pos_df, -5, HyperBonfP)
...
InterPro_pos_top <- top_n(InterPro_pos_df, -6, HyperBonfP)
...
###final plotting
#decided to keep "Mouse Phenotype", "GO Molecular Function" and "GO Biological Process" ontologies
#bind all three datasets together
whole_df <- rbind(Go_mol_df, GO_bio_df, mouse_df)
#make a new column with HyperBonferroni p values changed to log10
whole_df$p.adj <- -log10(whole_df$HyperBonfP)
#make new column with ratio of genes hit from pathway/totoal genes in pathway
whole_df$ratio <- whole_df$NumFgGenesHit/whole_df$TotalGenes

#save file as csv on computer
write.csv(whole_df,file="GREAT_df_location.csv")
...
whole_df_plot <- ggplot(data = whole_df, aes(x = direction, y = Desc, colour = whole_df$p.adj, size=whole_df$ratio )) + 
  ...
  #separate plot in 3 based on initial ontology datasets using facet_wrap, place in only one column
  facet_wrap(~Ontology, scales = "free", ncol = 1)
#view
whole_df_plot


####FINAL PLOT (FIGURE 3 IN PAPER)
#load cowplot to be able to merge all 3 plots in one figure
library(cowplot)

#to create this final figure you will need to have run the 4.1_EWAS_Island_effect script and have the "DNAm_Location_plot" in your environment
#you will also need to have run the 5.1_Manhattan_plot_Island_effect script and have the "manhplot" plot in your environment

#make the left part of Figure 3 with the manhattan plot and boxplots
...
Figure_3 <- plot_grid(left, whole_df_plot , labels = c('',  'C'), cols = 2, rel_widths = c(2, 1.25))
#view final Figure 3 and manually export as pdf
Figure_3
